...
setup_host_environment() {
    ...
    if [ ! -f "$ISO_FULL_PATH" ]; then
        ...
        local temp_iso_path="${ISO_DIR}/${UBUNTU_ISO_EXPECTED_FILENAME}.download"
        if wget -c -O "$temp_iso_path" "$UBUNTU_ISO_URL"; then
            log "Download successful. Renaming to $ISO_FULL_PATH."
            mv "$temp_iso_path" "$ISO_FULL_PATH"
            log "ISO $ISO_FULL_PATH is now available."
        else
            log "Error: Failed to download Ubuntu ISO from $UBUNTU_ISO_URL."
            rm -f "$temp_iso_path" # Clean up partial download
            exit 1
        fi
    else
        log "Ubuntu Desktop ISO already exists at $ISO_FULL_PATH."
    fi
    log "=== Host Environment Setup Complete ==="
}


check_project_files_remaining() {
    log "Checking remaining project files (VPN configs, scripts)..."
...
main() {
    ...
    setup_host_environment # Handles dependencies, KVM check, ISO download, user groups
    check_project_files_remaining # Check for VPNs and other scripts
